      ...
      <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg w-full max-w-4xl">
        ...
        {cartItems.length > 0 ? (
          <div>
            <ul>
              {cartItems.map((item) => (
                <li key={item.id} className="flex justify-between items-center mb-4 p-4 border-b border-gray-300 dark:border-gray-700">
                  ...
                  <div>
                    <p className="text-lg font-semibold text-gray-800 dark:text-white">Total: ${item.price * item.count}</p>
            ...
            <div className="mt-6">
              <h2 className="text-xl font-bold text-gray-800 dark:text-white">Apply Coupon</h2>
              <div className="flex items-center gap-2 mt-2">
                <input
                  type="text"
                  value={coupon}
            ...
            <div className="mt-6 text-right">
              <h2 className="text-xl font-bold text-gray-800 dark:text-white">Total Items: {totalItems}</h2>
              <h2 className="text-xl font-bold text-gray-800 dark:text-white">Total Price: ${totalPrice}</h2>
              <h2 className="text-xl font-bold text-gray-800 dark:text-white">Discount: ${discountAmount}</h2>
              <h2 className="text-xl font-bold text-gray-800 dark:text-white">GST (18%): ${gst.toFixed(2)}</h2>
              <h2 className="text-xl font-bold text-gray-800 dark:text-white">Delivery Charges: ${deliveryCharges}</h2>
              <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Final Total: ${finalTotal.toFixed(2)}</h2>
            </div>
          </div>
        ) : (
          <div className="text-center">
            <h2 className="text-xl text-gray-800 dark:text-white">Your cart is empty</h2>
